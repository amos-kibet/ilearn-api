...
router.get("/subject", (req, res) => {
  crawlerGET("subjects", (webData) => {
    ...
    for (const i in webData[req.query.school]) {
      data.push({
        code: i,
        name: webData[req.query.school][i]["name"],
...
router.get("/course", (req, res) => {
  crawlerGET(
    "current/fa/" + req.query.school + "/" + req.query.subject,
    (webData) => {
      const data = [];
      for (const i in webData) {
        for (const j in webData[i]["sections"]) {
          ...
          data.push({
            code: webData[i]["subjectCode"]["code"],
            ID:
              webData[i]["subjectCode"]["code"] +
              "-" +
              webData[i]["subjectCode"]["school"] +
              " " +
              webData[i]["deptCourseId"],
...
router.get("/search", (req, res) => {
  ...
  if (
    ...
    crawlerGET(
      req.query.year + "/" + req.query.semester + "/" + req.query.number,
      (webData) => {
        const data = [];
        for (const i in webData) {
          data.push({
